    ...
    module InstanceMethods
      ...
      def check_box_edit_support(view, tag_name, options)
        view.content_tag(
          ...
          :onclick => "javascript:" + options["method"] + "(\"" + tag_name + "\");"
        )
      end
      ...
      def check_box_edit_tag_with_checkall(view, tag_id, tag_name, custom_value, options = {})
        ...
        if custom_value.custom_field.multiple?
          ...
          checktags = check_box_edit_support(
            view,
            tag_name,
            {"label" => :label_check_all, "icon" => "icon-ok", "method" => "cfcbCheckAll"}
          )
          checktags += check_box_edit_support(
            view,
            tag_name,
            {"label" => :label_uncheck_all, "icon" => "icon-not-ok", "method" => "cfcbUncheckAll"}
          )
          if options[:class].start_with?("user_cf")
            ...
            selecttag += check_box_edit_support(
              view,
              tag_name,
              {"label" => :label_check, "icon" => "icon-ok", "method" => "cfcbCheckGroupAll"}
            )
            selecttag += check_box_edit_support(
              view,
              tag_name,
              {"label" => :label_uncheck, "icon" => "icon-not-ok", "method" => "cfcbUncheckGroupAll"}
            )
          ...
          filtertag += check_box_edit_support(
            view,
            tag_name,
            {"label" => :label_show_all, "icon" => "icon-zoom-in", "method" => "cfcbShowAll"}
          )
          filtertag += check_box_edit_support(
            view,
            tag_name,
            {"label" => :label_show_checked, "icon" => "icon-zoom-out", "method" => "cfcbShowChecked"}
          ) +
            ")"
